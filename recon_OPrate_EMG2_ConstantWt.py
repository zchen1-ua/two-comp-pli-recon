...
def event_likelihood(Hf, tau_m, sigma, fastRates, wts, rates):
    ...
    if (nr!=np.size(tau_m)) or (nc!=np.size(fastRates)) or \
        (1!=np.size(wts) or (nc!=np.size(rates))):
...
def _gradient_loglikehood(data, col_indices, row_ptr, tau, sigma, evlik,
    ...
    grad_wts=np.sum(grad_wts)
    return np.concatenate((np.array([grad_wts]),grad_rates))
...
def log_results(xc):
    global niter, likelihood_logged, NMSE_logged, grad_logged, weight_recon_save, crc_logged, bv_logged, alr_logged, std_alr_logged
    ...
    weight_recon_save = np.append(weight_recon_save, xc[0])
    ...
    print("step=", niter, "log_likelihood=", likelihood_logged[-1], "NMSE=", NMSE_logged[-1], "estimate of weight:", xc[0], "crc:", crc_logged[-1], "bv:", bv_logged[-1], "alr:", alr_logged[-1], "std_alr:", std_alr_logged[-1])
    if (niter in to_save):
        a = np.reshape(np.copy(rates),(N_obj,N_obj),order='C')
        rate_map_saved.append(a)
        #a = np.reshape(np.copy(wts),(N_obj,N_obj),order='C')
        a= np.copy(np.ones_like(OPrate_vec_true, dtype=np.float32)*xc[0])
        a[np.logical_not(f_vec_true>0)] = 0
        a = np.reshape(a, (N_obj,N_obj),order='C')
...
res = minimize(fun, merge2(np.array([OPwt_vec_recon]), OPrate_vec_recon), jac=jac,
...
if (niter not in to_save):
    ...
    wts = wts*np.ones_like(OPrate_vec_true, dtype=np.float32)
    wts[np.logical_not(f_vec_true>0)] = 0
...
import os
outpath = f"{wdir}/{TOF_FWHM}ps/" + str(run) + "/recon_OPrate_EMG2_FixedFastRate_SingleWt_with_Metrics/"
...
savemat(outpath+'recon_phantom5.mat',
        {'iters': saved,
         ...
         'OPwt_map_recon_vec': weight_recon_save,
...
# plot reconstructed weight vs iteration number
plt.figure()
plt.plot(1+np.arange(niter), weight_recon_save)
plt.axhline(y=0.3, color='r', linestyle='--')
plt.title("reconstructed weight")
plt.savefig(figures_outpath+"recon_weight_vs_iter.png")
