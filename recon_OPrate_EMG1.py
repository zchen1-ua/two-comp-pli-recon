# -*- coding: utf-8 -*-
"""
Maximum likelihood estimation of the rate-constant map of
the ortho-positronium (OP) lifespan pdf.

The pdf is assumed to contain a mixture of three EMG distributions
describing the direct-annihilation (DA) decay process,
the para-positronium (PP) decay process,
and the OP decay process.
The mixture-weight maps for all processes,
and the rate-constant maps for the DA and PP processes,
are assumed exactly known.
...
print (f"OP rate; EMG1, exactly known others: {run = }, {max_iters = }, {TOF_FWHM = }ps, {to_save = }")
...
def event_likelihood(Hf, tau_m, sigma, DAwts, DArates, PPwts, PPrates,
                     OPwts, OPrates):
    # check if HF is a csc matrix
    if not isspmatrix_csc(Hf):
        raise TypeError("Expect Hf a CSC matrix")
    nr, nc = Hf.shape
    if (nr!=np.size(tau_m)) or \
        (nc!=np.size(DAwts) or (nc!=np.size(DArates))) or \
        (nc!=np.size(PPwts) or (nc!=np.size(PPrates))) or \
        (nc!=np.size(OPwts) or (nc!=np.size(OPrates))):
        raise TypeError("Hf mismatch data dimension")
    re  = _event_likelihood(Hf.data, Hf.indices, Hf.indptr, tau_m, sigma, DAwts, DArates)
    re += _event_likelihood(Hf.data, Hf.indices, Hf.indptr, tau_m, sigma, PPwts, PPrates)
    re += _event_likelihood(Hf.data, Hf.indices, Hf.indptr, tau_m, sigma, OPwts, OPrates)
    return re

def loglikelihood(Hf, tau_m, sigma,
                  DAwts, DArates, PPwts, PPrates, OPwts, OPrates):
    return np.sum(np.log(event_likelihood(Hf, tau_m, sigma,
                     DAwts, DArates, PPwts, PPrates, OPwts, OPrates)))

@jit(nopython=True, parallel=True)
def _gradient_loglikehood(data, col_indices, row_ptr, tau, sigma,
                          evlik, wts, rates):
    n = np.shape(rates)[0]
    ...
    for row in np.arange(n):
        ...
        h = (1+erf(t3))/2
        d *= (1-rate*t2)*h-sigma*rate*np.exp(-t3**2)/np.sqrt(2*np.pi)
        grad[row] = np.sum(data[row_ptr[row]:row_ptr[row+1]]*d)
    return grad

def gradient_loglikehood(Hf, Hf_t, tau_m, sigma, DAwts, DArates,
                         PPwts, PPrates, OPwts, OPrates):
    # check if HF is a CSC matrix
    if not isspmatrix_csc(Hf):
        raise TypeError("Expect Hf a CSC matrix")
    ...
    evlik = event_likelihood(Hf, tau_m, sigma, 
                             DAwts, DArates, PPwts, PPrates, OPwts, OPrates)
    return _gradient_loglikehood(Hf_t.data, Hf_t.indices, Hf_t.indptr,
                            tau_m, sigma, evlik, OPwts, OPrates)
...
def fun(x, Hf, Hf_t, tau, sigma, DAwts, DArates, PPwts, PPrates, OPwts):
    return -loglikelihood(Hf, tau, sigma, DAwts, DArates, PPwts, PPrates, OPwts, x)
def jac(x, Hf, Hf_t, tau, sigma, DAwts, DArates, PPwts, PPrates, OPwts):
    return -gradient_loglikehood(Hf, Hf_t, tau, sigma, DAwts, DArates,
                                 PPwts, PPrates, OPwts, x)

## prepare initial solution for OP lambda
...
def log_results(xc):
    global niter, images_saved, likelihood_logged, NMSE_logged, grad_logged, crc_logged, bv_logged, alr_logged, std_alr_logged
    global Hf, Hf_t, tau_m, tau_sigma
    global DAwt_vec_true, DArate_vec_true
    global PPwt_vec_true, PPrate_vec_true
    global OPwt_vec_true
    likelihood_logged.append(-fun(xc, Hf, Hf_t, tau_m, tau_sigma,
                                  DAwt_vec_true, DArate_vec_true,
                                  PPwt_vec_true, PPrate_vec_true,
                                  OPwt_vec_true))
    NMSE_logged.append(NMSE(xc, OPrate_vec_true))
    grad_logged.append(LA.norm(jac(xc, Hf, Hf_t, tau_m, tau_sigma,
                                   DAwt_vec_true, DArate_vec_true,
                                   PPwt_vec_true, PPrate_vec_true,
                                   OPwt_vec_true)))
    crc_logged.append(calculate_crc(OPrate_vec_true, xc))
    bv_logged.append(calculate_bv(OPrate_vec_true, xc))
    alr_logged.append(calculate_abs_log_ratio(OPrate_vec_true, xc, ind_circle1, ind_circle2, ind_background))
    ...
    if (niter in to_save):
        im = np.reshape(np.copy(xc),(N_obj,N_obj),order='C')
        images_saved.append(im)
    niter += 1
    return

## call minimizer
res = minimize(fun, OPrate_vec_recon, jac=jac,
               args=(Hf, Hf_t, tau_m, tau_sigma, DAwt_vec_true, DArate_vec_true,
                     PPwt_vec_true, PPrate_vec_true, OPwt_vec_true), \
    method='L-BFGS-B', bounds=[(0,np.inf)]*np.size(OPrate_vec_recon), \
    callback=log_results, options={'maxiter': max_iters, 'ftol': ftol, 'disp': 10})
OPrate_vec_recon = res.x

niter -= 1
if (niter not in to_save):
    to_save = np.append(to_save, niter)
    images_saved.append(np.reshape(OPrate_vec_recon,(N_obj,N_obj),order='C'))
saved = np.unique(to_save[to_save<=niter])

#---------------------------------
# save and show results
import os
outpath = f"{wdir}/{TOF_FWHM}ps/" + str(run) + "/recon_OPrate_EMG1_ExactlyKnownOthers_with_Metrics/"
...
plt.gcf().canvas.manager.set_window_title("true activity and rate images")
plt.savefig(figures_outpath + "activity_OPrate_true.png",dpi=300,bbox_inches='tight')
...
for (n,im) in zip(saved, images_saved):
    ...
    plt.colorbar()
    plt.clim([0,None])
    plt.title(f"reconstructed map of OP rate, iter {n}")
    plt.gcf().canvas.manager.set_window_title(f"iter {n}")
    plt.savefig(figures_outpath + f"OPrate_recon_iter{n}.png")